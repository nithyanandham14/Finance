...
public class Itemservice {
    ...
    public Item getItemWithInterestDetails(int jwlno, double monthlyRate, LocalDate currentDate) {
        ...
        if (daysBetween % 30 > 5) months++; // follow 5-day rule

        double principal = item.getPrincipalamt();

        double simpleInterest = principal * monthlyRate * months;
        ...
        if (months <= 12) {
            ...
            // Remaining months are calculated with SI on latest compound principal
            double siRemaining = compoundPrincipal * monthlyRate * extraMonths;
